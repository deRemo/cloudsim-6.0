...
public class MyTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// Create vmlist
			vmlist = createVMs(broker.getId(), 1);
			...
			// create VM
			NetworkVm vm = new NetworkVm(vmid, broker.getId(), mips, pesNumber, ram, bw, size, vmm, new CloudletSchedulerTimeShared());
			...
			// Fifth step: Create one cloudlet app
			applist = new ArrayList<>();
			AppCloudlet app = new AppCloudlet(1, 0, 30, 1, 1);
			applist.add(app);
			//app.createCloudletList(vmlist);


			broker.setAppCloudletList(applist);
			//Prob not needed: broker.submitCloudletList(app.clist);
			

			for (Cloudlet c : broker.getCloudletSubmittedList()) {
				Log.printLine("submitted cloudlet_id: "+c.getCloudletId());
			}

			
			for (Cloudlet c : broker.getCloudletReceivedList()) {
				Log.printLine("received cloudlet_id: "+c.getCloudletId());
			}
			...
			Log.printLine("MyTest finished!");


			Log.printLine("Datacenter characteristics:");

			for (Host host : datacenter0.getHostList()) {
				Log.printLine("host-id: "+host.getId()+" #pe: "+host.getNumberOfPes());
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private static NetworkDatacenter createDatacenter(String name) {
		...
		for (int i = 0; i < NetworkConstants.EdgeSwitchPort * NetworkConstants.AggSwitchPort
			...
			// 2. A Machine contains one or more PEs or CPUs/Cores.
			// In this example, it will have 8 cores
	...
	private static List<NetworkVm> createVMs(int userId, int vms) {

		//Creates a container to store VMs. This list is passed to the broker later
		LinkedList<NetworkVm> list = new LinkedList<>();
